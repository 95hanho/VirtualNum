<template>
    <div class="setInput">
            ...
            <div class="page-content" id="acsSetPageContent">
                <div class="row">
                    <div class="widget-header widget-header-small">
                        <h5 v-if="!updating&&!prioritizing" class="lighter">IBIZ 설정 등록</h5>
                        <h5 v-if="updating&&!prioritizing" class="lighter" style="color:red;">IBIZ 설정(Idx={{ setIdx }}) 수정</h5>
                        <h5 v-if="prioritizing" class="lighter" style="color:orange;">IBIZ 설정 우선순위 변경 중</h5>
                    </div>
                        ...
                        <div class="widget-main">
                            <div class="row" style="height:70px;">
                                <div class="input-group col-xs-3">
                                    ...
                                    <div class="input-group bootstrap-timepicker">
                                        ...
                                        <input class="form-control" v-model="setName" @keyup.enter="setInput" ref="setNameRef" :disabled="prioritizing">
                                    </div>
                                </div>
                                <div class="input-group col-xs-3">
                                    <label>설정 타입</label>
                                    <div class="input-group bootstrap-timepicker">
                                        ...
                                        <select class="form-control" v-model="setType" :disabled="prioritizing">
                            ...
                            <div class="row" style="height:70px;">
                                <div class="input-group col-xs-6">
                                    <label>스케줄</label>
                                    <div class="input-group bootstrap-timepicker" @click="showView(2)">
                                        <span class="input-group-addon">
                                            <i class="icon-time bigger-110"></i>
                                        </span>
                                        <input class="form-control" v-model="schedule" style="cursor:pointer;" disabled>
                                    </div>
                                </div>
                                <div class="input-group col-xs-6">
                                    <label>VNS 번호</label>
                                    <div class="input-group bootstrap-timepicker" @click="showView(3)">
                                        <span class="input-group-addon">
                                            <i class="icon-phone bigger-110"></i>
                                        </span>
                                        <input class="form-control" v-model="vns" style="cursor:pointer;" disabled>
                                    </div>
                                </div>
                            </div>
                            <div class="row" style="height:90px;">
                                <div class="input-group col-xs-6">
                                    <label>착신 번호</label>
                                    <div class="input-group bootstrap-timepicker" @click="showView(4)">
                                        <span class="input-group-addon">
                                            <i class="icon-phone bigger-110"></i>
                                        </span>
                                        <input class="form-control" v-model="called" style="cursor:pointer;" disabled>
                                    </div>
                                </div>
                                <div class="input-group col-xs-6">
                                    <label>멘트</label>
                                    <div class="input-group bootstrap-timepicker" @click="showView(5)">
                                        <span class="input-group-addon">
                                            <i class="icon-play-circle bigger-110"></i>
                                        </span>
                                        <input class="form-control" v-model="ment" style="cursor:pointer;" disabled>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="input-group pull-left col-xs-3">
                                    <button v-if="!prioritizing" @click="showView(1)" class="btn btn-primary btn-sm callDiv" style="margin-left:5px;" >
                                        설정 조회<i class="icon-search icon-on-right bigger-110"></i>
                                    </button>
                                    <button v-if="setView&&!prioritizing&&!updating" @click="prioritize" class="btn btn-info btn-sm callDiv" style="margin-left:5px;width:50%;">
                                        WEIGHT 변경<i class="icon-rotate-right icon-on-right bigger-110"></i>
                                    </button>
                                </div>
                                <div class="input-group pull-right col-xs-3">
                                    <div class="input-group" id="inputBtn">
                                        <button v-if="!updating&&!prioritizing" @click="formReset" class="btn btn-primary btn-sm callDiv" style="margin-left:5px;" >
                                            리셋<i class="icon-download icon-on-right bigger-110"></i>
                                        </button>
                                        <button v-if="!updating&&!prioritizing" @click="setInput" class="btn btn-success btn-sm callDiv">
                                            입력<i class="icon-magic icon-on-right bigger-110"></i>
                                        </button>

                                        <button v-if="updating&&!prioritizing" @click="updateCancel" class="btn btn-danger btn-sm callDiv" style="margin-left:5px;" >
                                            취소<i class="icon-download icon-on-right bigger-110"></i>
                                        </button>
                                        <button v-if="updating&&!prioritizing" @click="updateSet" class="btn btn-warning btn-sm callDiv">
                                            수정<i class="icon-magic icon-on-right bigger-110"></i>
                                        </button>

                                        <button v-if="prioritizing" @click="prioritizeCancel" class="btn btn-danger btn-sm callDiv" style="margin-left:5px;" >
                                            그만하기<i class="icon-download icon-on-right bigger-110"></i>
                                        </button>
                                    </div>
                                </div>
                            </div>
        ...
        <SetList v-show="setView" :setObject="setObject" :updateSetIdx="setIdx" @setUpdateForm="setUpdateForm"/>
...
export default {
    ...
    created(){
        ...
        EventBus.$on('pickMent', (pickList) => {
            ...
            if(pickList.length == 0){
                ...
                this.ment = "[" + pickList[0].mentName + ', ' + pickList[0].type + '타입]';
                for(let key in pickList){
                    if(key > 0){
                        this.ment += ", [" + pickList[key].mentName + ', ' + pickList[key].type + '타입]';
                    }
                }
            }
    ...
    methods:{
        async allselect(){
            ...
            await this.$http.post('/set/select?sequence='+ sequence +'&responseType=json', message)
            .then(res => {
            ...
            await this.$http.post('/schedule/select?sequence='+ sequence +'&responseType=json', message)
            .then(res => {
                if(res.data.result.resultCode == '00'){
            ...
            this.$store.commit('autoSequence');
            sequence = this.$store.state.login.sequence;
            await this.$http.post('/number/ibiz/vns/select?sequence='+ sequence +'&responseType=json', message)
            .then(res => {
                if(res.data.result.resultCode == '00'){
            ...
            this.$store.commit('autoSequence');
            sequence = this.$store.state.login.sequence;
            await this.$http.post('/number/ibiz/called/select?sequence='+ sequence +'&responseType=json', message)
            .then(res => {
                if(res.data.result.resultCode == '00'){
                    if(res.data.calledNumberList.length == 0){
                        ...
                        this.calledNumberObject.calledNumberList = res.data.calledNumberList;
                    }
            ...
            this.$store.commit('autoSequence');
            sequence = this.$store.state.login.sequence;
            await this.$http.post('/ment/select?sequence='+ sequence +'&responseType=json', message)
            .then(res => {
                if(res.data.result.resultCode == '00'){
        ...
        setSelect(){
            ...
            this.$http.post('/set/select?sequence='+ sequence +'&responseType=json', message)
            .then(res => {
                if(res.data.result.resultCode == '00'){
                    if(res.data.setList.length == 0){
                        this.setObject.zeroList = true;
        ...
        setInput(){
            ...
            if(this.setName.length == 0){
                ...
                this.$store.commit('autoSequence');
                const sequence = this.$store.state.login.sequence;
                this.$http.post('/set/insert?sequence='+ sequence +'&responseType=json', message)
                .then(res2 => {
        ...
        updateSet(){
            ...
            if(this.setName.length == 0){
                ...
                this.$store.commit('autoSequence');
                const sequence = this.$store.state.login.sequence;
                this.$http.post('/set/update?sequence='+ sequence +'&responseType=json', message)
                .then(res2 => {
                    if(res2.data.result.resultCode == '00'){
                        alert(res2.data.result.resultMessage);
                        this.updating = false;
