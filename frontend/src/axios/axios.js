...
axios.interceptors.request.use(async(config) =>{
    ...
    // **** Real Ibiz API ****
    //config.url = 'http://172.16.80.113:8090' + config.url;
    ...
    */
    ...
    // 로그인 일때를 제외
    if(!config.url.includes('encrypt/state') && !config.url.includes('user/login') && !config.url.includes('sequence/select')
    && !config.url.includes('upload/ment')){
        
        // aes 확인 => on 일 시 data를 aes128암호화
        const aesOn = VueCookies.get('aesOn');
        if(aesOn == 'on'){
            await store.dispatch('aes128', config.data)
            .then((res) => {
                config.data = res;
            });
        }
        // 요청이 있을 때 마다 쿠키 refresh
        if(VueCookies.get('loginMarker') !== null && VueCookies.get('userId') !== null
        && VueCookies.get('url') !== null && VueCookies.get('clientCode') !== null){
